...
> 文档范围：自研 `nginx-rtc-module` 的组件映射、桥接层、RTP 封装、信令与媒体安全、UDP 服务与 nginx stream 集成、状态机/RTCP/GOP 缓存，以及零拷贝边界与多 worker 扩展。每个主题给出关键数据结构、函数 API 与 Mermaid 图。
...
原则：**逻辑下沉到纯 C 模块，nginx 侧只做事件/配置/socket 翻译**。纯 C 单测用模块仓的 `test/Makefile`：Linux 下 `NGX_SRC` 默认指向已 configure 的 openresty/nginx 源码树，生产 TU 直接用真实 nginx 头编译；`test/nginx_stub.c` 在真实头构建里只补 nginx 未链接的符号，无 nginx 头（Windows 交叉构建）时它才提供完整的桩对象。RFC 位级代码（STUN 长度边界、FU-A 位级、SDP 解析）必须有单测覆盖，端到端测试只数包不校验语义，不足以暴露这类缺陷。

### 1.4 代码布局与编译归属

| 文件 | 类型 | 归属 | 说明 |
| --- | --- | --- | --- |
| `ngx_rtmp_rtc_bridge_module.c` | CORE 胶水 | nginx RTMP | 广播明文 RTP |
| `ngx_rtc_http_module.c` | HTTP 胶水 | nginx HTTP | 信令 |
| `ngx_rtc_stream_module.c` | STREAM 胶水 | nginx stream | UDP STUN/DTLS/SRTP |
| `ngx_rtc_core.c/.h` | 纯 C | 注册表 + 本地 GOP ring + 发送 pacing/TWCC AIMD | source rbtree、session/订阅 ngx_queue、GOP ring、pacing/TWCC AIMD、NACK 窗口退避 |
| `ngx_rtc_core_module.c` | CORE 胶水 | nginx core | `rtc_zone` 指令与 core 模块定义 |
| `ngx_rtc_shm.c/.h` | 纯 C + nginx 类型 | shm 注册表 | shm source/session 骨架、publish 所有权、跨 worker 重传环/媒体环、reaper |
| `ngx_rtc_ring.c/.h` | 纯 C | 定长环 / 变长字节环 | `ngx_rtc_vring_t` 作音频转码线程收发队列 |
| `ngx_rtc_rtp.c/.h` | 纯 C | RTP 封装 | RFC 6184 single/STAP-A/FU-A，B 帧过滤 |
| `ngx_rtc_sdp.c/.h` | 纯 C | SDP | offer 解析 + answer 生成 |
| `ngx_rtc_stun.c/.h` | 纯 C | STUN | BindingRequest/Response |
| `ngx_rtc_dtls.c/.h` | 纯 C | DTLS | 握手 + SRTP 密钥导出 |
| `ngx_rtc_srtp.c/.h` | 纯 C | SRTP | libsrtp2 protect/unprotect |
| `ngx_rtc_audio.c/.h` | 纯 C | AAC→Opus | FFmpeg aac + libswresample + libopus |
| `ngx_rtc_audio_worker.c/.h` | pthread | 音频转码线程 | vring + mutex/cond 隔离 FFmpeg |
| `ngx_rtc_jitter.c/.h` | 纯 C | WHIP 上行重排 | 按 seq 有序出队、gap 超时跳过 |
| `ngx_rtc_avsync.c/.h` | 纯 C | SR→NTP 映射 | 已实现并单测覆盖，当前无运行时调用方 |
| `ngx_rtc_rtcp.c/.h` | 纯 C | RTCP 编解码 | bridge 周期 SR；stream NACK/PLI/TWCC/BYE |
| `ngx_rtc_hsm.c/.h` + `ngx_rtc_session_fsm.c/.h` | 纯 C | 状态机 | session 生命周期纯状态跟踪 |
...
关键符号索引（nginx-http-flv-module；只给符号名，行号随版本变动）：

| 符号 | 文件 |
| --- | --- |
| `ngx_rtmp_receive_message`（事件分发循环） | `ngx_rtmp_handler.c` |
| `ngx_rtmp_fire_event` | `ngx_rtmp.c` |
| `ngx_rtmp_codec_av` / parse avc/aac header | `ngx_rtmp_codec_module.c` |
| `ngx_rtmp_gop_cache_av`（publisher 帧钩子范本） | `ngx_rtmp_gop_cache_module.c` |
| `ngx_rtmp_live_av`（直播分发） | `ngx_rtmp_live_module.c` |
| 事件 handler 类型 / events 数组 | `ngx_rtmp.h` |
...
音频 RTP 时间戳源级单调：首个裸 AAC tag 播种 48k 时钟（`audio_ts_valid`），此后每成功 emit 一帧 Opus 由 `ngx_rtmp_rtc_audio_frame` 回调 `audio_ts += NGX_RTC_AUDIO_OPUS_FRAME_SIZE(960)`，而不是每个 AAC tag 复用同一次 tag 时间戳——一帧 AAC（1024 样本）经 FIFO 可转出两帧 Opus，共用时间戳会被对端按时间戳去重、规律性丢 20 ms 音频。
...
三条路径共用同一套 HMAC 令牌——`<app>/<stream>|t=<过期秒>` 的 base64url HMAC-SHA256 签名；**密钥按 `app/stream` 各自一份**，查 `lua_shared_dict stream_keys`（由 `config.lua` 每 10 s 从 `stream_keys.lua` 热加载）：
...
> 状态机的定位是「纯状态跟踪」，真实动作（DTLS 建链、SRTP 导出、订阅/退订、teardown）留在 `ngx_rtc_stream_module.c`。
...
| 组件 | 决策 | 说明 |
| --- | --- | --- |
| `lua_shared_dict` | **用** | 共享状态载体：`stream_keys` / `rate_limit` / 统计镜像 |
| `resty.limit.req`（lua-resty-limit-traffic） | **用** | 信令限流，rate/burst 漏桶语义 |
| cosocket | **可用** | 外接认证网关、`on_publish` 回调查外部服务 |
| lua-resty-lock | **可用** | 多 worker 协调、共享状态原子更新 |
| lua-resty-redis / mysql | **不用** | 不引入外部存储；密钥本地表 + shared dict 热加载（10 s `ngx.timer.every`） |
| session 超时/回收 | **保持在 C 模块** | 归 stream 模块 reap timer；Lua 只做信令/限流/配置，不重复维护 session 状态 |

分界线：**信令与控制面**（鉴权、限流、路由、配置热加载）在 Lua；**媒体面与协议栈**在纯 C
模块——没有可承载 WebRTC 媒体面的 `lua-resty-webrtc/sdp/dtls/srtp` 模块，纯 Lua 拼媒体栈不可控。
...
| **命令行 reload 劈开媒体面** | 即使只改配置，`nginx -s reload` 也会 fork 新 worker 而**旧 worker 因长连接 RTMP 发布会一直"shutting down"**：推流者留在旧 worker，UDP 8000 的监听已归新 worker。实测一次 reload 后 `/rtc/v1/play/` 信令仍 200，但端到端 **0/4** 零媒体包；`run.sh stop` + `run.sh start` 立刻恢复 4/4 | 这套多 worker + shm 架构下**改配置也要全量重启**，不要用 reload。判据：`ps` 里出现 `worker process is shutting down` 同时 `ss -lunp` 的 8000 已指向新 pid |
| 事件循环内同步 CPU 密集工作 | 单 worker 下多路推流互相挤压，握手/分发延迟 | 音频转码独立 pthread + 有界环（nginx-srt 模式）；周期任务走 `ngx_event_timer`，见 5.2 / 6.3 |

## 11 验收口径

端到端以 werift 无头播放器收包为准（`audio.pkts>0 && video.pkts>0` 即 PASS），
另观测首个 H264 IDR 到达时间（`first_keyframe_ms`，真实出图延迟口径）。
