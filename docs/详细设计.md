...
> 文档范围：自研 `ngx-rtc-module` 的总体架构与组件映射、桥接层、RTP 封装、信令与媒体安全、UDP 服务与 nginx stream 集成、状态机/RTCP/GOP 缓存，以及零拷贝边界、多 worker 扩展、开源先例与落地路线。每个主题给出关键数据结构、函数 API 与 Mermaid 图。
