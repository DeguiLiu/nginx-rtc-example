...
| 信令鉴权（HMAC 签名 + 限流） | Lua `auth.lua`（`resty.limit.req` 官方限流），`t`/`sign` 校验通过返回 `code=0` |
| 推流/播放三路鉴权 | RTMP `on_publish` + HTTP-FLV `flv_auth.lua` + `/rtc/v1/play/`，同一套 `t`/`sign` 令牌 |
| WHIP 上行 | `rtc_whip` 端点；`ngx_rtc_jitter` 上行重排缓冲 |
| RTCP（SR/RR/SDES/NACK/PLI/BYE + TWCC） | bridge 2s 周期 SR+SDES；stream 解码 SRTCP 并驱动 NACK/PLI/BYE/TWCC；上行方向另发 PLI 向推流端要关键帧 |
...
| 多 worker 共享内存 | `rtc_zone` + `ngx_rtc_core_module` + shm 骨架/媒体环，部署 `worker_processes 2`；见 `multi-worker-shm-design.md` |
| 统计端点 | `/rtc/v1/stats`（`conf/stats.lua` 读 `rtc_stats` shdict，C 模块每秒镜像，无内部子请求） |
| 纯 C 协议核心 host 单测 | 独立仓 `nginx-rtc-module/test/`，Linux 下按真实 nginx 头文件构建 |
...
- 纯 C 协议核心（`rtp/sdp/stun/dtls/srtp/audio/rtcp/avsync/jitter/hsm/session_fsm/core/ring/shm`）**不引用 nginx 头文件、无全局可变状态**，输入输出全部经参数传入（`emit`/`rtcp_decode_cb` 回调、调用者持有的 scratch buffer），因此可脱离 nginx 在 host 上单测（测试在独立仓 `nginx-rtc-module/test/`）。`core` 注册表在 nginx 编译下使用 `ngx_rbtree`/`ngx_queue`；`shm` 在 host 单测下由 slab/shmtx stub 提供。
...
一次典型端到端运行（640x360 H264 + AAC 源，`client/play.mjs` 拉流）：

| 量 | 值 |
| --- | --- |
| 视频 H264 接收 | 761 包 / 362 KB |
| 音频 Opus 接收 | 389 包 / 67 KB |
| 首包延迟 | 约 560 ms |

数字随客户端、时长与码流变化，引用时标注运行条件。
...
| SRTP | 视频/音频 RTP 加密传输 |
| B 帧过滤 | `-bf 0` 推流，过滤逻辑生效 |
| HTTP-FLV 兜底 | HTTP 200，FLV 头正确 |
...
| 鉴权 | OpenResty `access_by_lua_file`/`on_publish` + `lua_shared_dict` | 推流/HTTP-FLV/信令三路共用同一套 `t`/`sign` HMAC 令牌 + 信令 IP 限流（`resty.limit.req`）+ 10 s 定时热加载，免 reload |
...
- 鉴权三路共用同一套令牌：RTMP 推流端经 `on_publish` 回调 `rtmp_auth.lua`、HTTP-FLV 取流经 `/live` 的 `access_by_lua_file conf/flv_auth.lua`、WebRTC 信令经 `/rtc/v1/play/` 的 `auth.lua`。令牌为 `<app>/<stream>|t=<过期秒>` 的 base64url HMAC-SHA256 签名（`config.lua` / `hmac.lua`），**密钥按 `app/stream` 各自一份**（`stream_keys.lua`），不是全局同一把；密钥存 `lua_shared_dict`，由 `config.lua` 每 10 s 热加载；admin API（loopback-only）管理密钥。
...
- **Windows 打包**：产物在 `dist/win64-package/`（含自带 `conf/nginx.conf`）；构建流程见 `编译文档.md` 的 Windows 一节。

## 11 未实现项

架构层面的未实现能力与已知缺陷集中在模块仓 `nginx-rtc-module/docs/BACKLOG.md`；本仓只需知道：
进程内完整注册表与 shm 骨架目前是双镜像（registry 本体尚未迁入 zone），
跨 worker 重传环仍走 slab pool mutex，其成本指标是 `/rtc/v1/stats` 的 `retx_lock.*`。
