...
> 文档范围：自研 `ngx-rtc-module`（挂在 OpenResty 1.31.1.1 之上的 RTMP→WebRTC 桥接）的整体架构、模块分层、进程/端口模型与设计原则。正文全部基于本仓库真实代码与已验证事实，不确定性内容标注「待验证」。
> 关联代码：`/home/dgliu/workspace/webrtc/ngx-rtc-module/`；配套文档：`详细设计.md`、`编译文档.md`、`测试文档.md`。
> 已与代码同步于 2026-09-09。

**结论摘要**：方案在 OpenResty 1.31.1.1（nginx 1.31 底座）进程内，通过三个 nginx 胶水模块（RTMP 桥接 / HTTP 信令 / UDP 媒体）加一组纯 C11 核心单元（rtp/sdp/stun/dtls/srtp/audio/rtcp/hsm/session_fsm/core），实现「RTMP 推流 → H264 NALU 拆解与 AAC→Opus 转码 → RTP 封装 → SRTP → UDP 8000 → WebRTC 播放器」的低延迟链路，同时保留 HTTP-FLV 兜底。端到端已验证：视频 H264 761 包/362KB、音频 Opus 389 包/67KB、首包约 560 ms、信令鉴权 `code=0`、服务端 candidate 为局域网地址 `172.16.48.122:8000`。RTCP 反馈（2s SR + NACK/PLI/BYE）、GOP 环形缓存、session HSM 状态机均已集成进 bridge/stream/core；RTMP 推流、HTTP-FLV 取流与 `/rtc/v1/play/` 三路 stream key 鉴权已闭合；多 worker 共享内存仍为未来阶段设计。

## 1 结论与定位

### 1.1 一句话定位

在 **nginx 框架内**（而非独立 RTC 服务）完成 RTMP 直播源到 WebRTC 播放的低延迟转换，是相对 SRS 等独立服务方案的核心差异点，也是本方案专利创新点所在。

### 1.2 已验证能力边界

| 能力 | 状态 | 依据 |
| --- | --- | --- |
| RTMP 接入 + HTTP-FLV 兜底 | 已验证 | nginx-http-flv-module，RTMP :1935，HTTP-FLV :18082（HTTP 200） |
| H264 → RTP（single/STAP-A/FU-A + B 帧过滤） | 已验证 | `ngx_rtc_rtp.c`；实测 761 包 / 362 KB |
| AAC → Opus 转码并封装 RTP | 已验证 | `ngx_rtc_audio.c` + `ngx_rtc_opus_packetize`；实测 389 包 / 67 KB |
| STUN/DTLS/SRTP 媒体建链 | 已验证 | 局域网 candidate `172.16.48.122:8000`，首包约 560 ms |
| 信令鉴权（stream key + 限流） | 已验证 | Lua `auth.lua`，正确 key 返回 `code=0` |
| 推流/播放三路鉴权 | 已验证 | RTMP `on_publish` + HTTP-FLV `flv_auth.lua` + `/rtc/v1/play/` 同源 stream key |
| RTCP（SR/RR/SDES/NACK/PLI/BYE） | 已集成 | bridge 2s 周期 SR+SDES；stream 解码 SRTCP 并按 NACK/PLI/BYE 动作 |
| session HSM 状态机 | 已集成 | `ngx_rtc_session_fsm` 纯状态跟踪；source 状态机已删除（bridge `publishing` 为准） |
| GOP 环形缓存 / NACK 重传 / 订阅即发 | 已集成 | `source->gop` 2048 槽环形缓存明文 RTP；DTLS 完成回放、NACK/PLI 走 ring |
| 纯 C 协议核心 host 单测 | 已验证（37/37 PASS） | `ngx-rtc-module/test/`，覆盖 rtp/stun/sdp/rtcp/hsm/session_fsm |
| 多 worker 共享内存 | 未来阶段 | 见 `docs/multi-worker-shm-design.md` |

### 1.3 端到端延迟预算（实测口径）

首包约 560 ms 的构成未做逐段拆分（待验证）；已确认链路为全 UDP 出网、IDR 前以 STAP-A 携带 SPS/PPS。服务端已实现 GOP 环形缓存：新订阅者在 DTLS/SRTP 就绪并订阅时先回放最近一个 GOP（自最新 IDR 的 STAP-A 起），无需等待下一个 IDR；真实出图延迟以播放器首帧 IDR 到达口径 `first_keyframe_ms` 衡量（见测试文档）。
...
| | `ngx_rtc_rtcp` | RTCP SR/RR/SDES/NACK/PLI/BYE 编解码（已集成） | `srs_kernel_rtc_rtcp.cpp` |
| | `ngx_rtc_hsm` + `ngx_rtc_session_fsm` | 层次状态机引擎 + session 生命周期状态机（已集成，纯状态跟踪） | SRS 连接生命周期 |
...
- STUN 短期凭据使用 answer 下发的服务端 `ice-ufrag/ice-pwd` 与客户端 offer 携带的 ufrag 组成 `server:client` username，`MESSAGE-INTEGRITY` 用本地 `ice_pwd` 计算，`FINGERPRINT` 校验未实现（`待验证`是否需要）。
