...
## 7 附：SR 跨 worker 计数为零的缺陷（已修复）

### 7.1 缺陷

`ngx_rtmp_rtc_send_sr_sdes()`（`ngx_rtmp_rtc_bridge_module.c`）发送 SR 时读取的是**本 worker 的** `ngx_rtc_source_t`：
...
而 shm → 本地 source 的回填点（`ngx_rtmp_rtc_bridge_module.c`、`ngx_rtc_stream_module.c`、`ngx_rtc_http_module.c` 共四处）**只复制 `video_ssrc / audio_ssrc / video_pt / audio_pt`，从不复制 `video_ts / video_pkts / video_octets`**。这三个字段只在推流端 worker 的包处理路径上维护；`ngx_rtc_shm_source_t` 虽然声明了 `video_ts`，但全仓没有任何写入者。
...
影响：客户端用 SR 建立 RTP↔NTP 映射、估算发送码率与做 A/V 同步；`rtp_ts=0` 会让基于它的时间映射整体失真。注意 `/rtc/v1/stats` 读的是 `shm_src->video_pkts`（正确来源），所以该接口不受影响——SR 是这条路径上的例外。

### 7.2 修复

在 `ngx_rtc_shm.h/.c` 增加一对 host 可测的小函数，bridge 调用它们：

- `ngx_rtc_shm_source_set_video()` —— 发布端每包写入 `video_ts / video_pkts / video_octets`，锁外写（沿用「计数字天然对齐、跨 worker 读不撕裂」的既有约定）。原先各写一遍这两组计数的 `shm_stats()`（bridge 与 stream_module 各一处）收敛到这一个入口，顺带补上从未被写过的 `video_ts`。
- `ngx_rtc_shm_source_sr_video()` —— 解析报告该带什么：镜像有数据就用镜像；否则回退本地（单 worker 部署没有 shm，本地就是发布者自己）；两边都没有则返回 0。

`send_sr_sdes()` 每次发报告前用 `ngx_rtc_shm_source_get()` 查一次镜像并解析，解析不出来就**不发**——一个时间戳从未上线过的 SR 比不发更糟，客户端会拿它当锚点。不缓存指针，因为观看端 worker 不持有 publish claim，缓存会被 reaper 释放成悬垂指针。

### 7.3 验证

宿主单测：`test/test_shm.c` 新增三个用例（镜像优先 / 无镜像回退本地 / 未出帧时拒绝），先看到编译失败（`implicit declaration`）再实现；real 与 stub 两个 header world 均 `TOTAL: 155  PASS: 155  FAIL: 0`。

跨 worker 路径用隔离 prefix 做 A/B（共享 prefix 被另一会话占用，故另起实例，端口 11935 / 28082 / UDP 18000，仅按自身 pidfile 启停，不触碰共享实例）。同一 prefix、同一配置、同一推流，只换二进制；落点由日志判定——SR 由**持有播放端 UDP socket 的媒体 worker** 发出，因此与 avsync 日志里的推流端 worker pid 比较，而不是与处理 HTTP 信令的 worker 比（判据取错会得出完全相反的结论）：

| 二进制 | 落点 | 观测到的 SR `rtpTs` |
| --- | --- | --- |
| 修复前 | 跨 worker（3/3 次） | 全部为 0 |
| 修复前 | 同 worker（3/3 次） | 正常且随时间递增 |
| 修复后 | 跨 worker（2/2 次） | 正常且随时间递增，0 条为零 |
| 修复后 | 同 worker（4/4 次） | 正常且随时间递增，0 条为零 |

每次运行 3 条 SR，落点与结果完全相关：修复前跨 worker ⟺ 全 0，修复后无论落点均为有效值。

未覆盖的部分：bridge 的接线（镜像查找与"无数据不发"）本身没有单测——该模块依赖第三方 `ngx_rtmp*.h`，进不了 host 套件，只有编译保证。
